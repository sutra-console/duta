...
// ---- mock board: compiled default (2 outputs) + mcu map + board overlay -----
...
static const duta_pin_use duta_board_uses[] = {{8, DUTA_USE_DUAL, "onboard LED"}};
#define DUTA_USES_N 1
...
int main(void) {
  ...
  // resolver: offerable pins are 4,5,6,8 (7 is forbidden -> hidden). total = 4.
  ...
  duta_io_begin();
  assert(duta_tbl_n == 2 && duta_tbl[0].pin == 4);
  uint8_t t, fl; uint16_t arg;
  assert(duta_io_config_get(0, 0, &t, &pin, &fl, &arg, &nm) == 2 && t == SKRIT_CTRL_IO && pin == 4);
  ...
  duta_tbl = duta_outputs; duta_tbl_n = 2; // simulate a power-cycle (static init state)
  duta_io_load();
  assert(duta_tbl_n == 2 && duta_tbl[0].pin == 4);
